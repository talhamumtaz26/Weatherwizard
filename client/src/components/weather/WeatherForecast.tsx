    ...
    >
      ...
      <div className="space-y-4">
        {forecast.map((day, index) => (
          <motion.div
            key={day.date}
            className="flex items-center justify-between py-3 border-b border-white/20 last:border-b-0 hover:bg-white/10 rounded-lg px-2 transition-colors cursor-pointer"
          ...
          >
            <div className="flex items-center flex-1">
              <div className="w-16 text-sm font-medium text-white/90">
                {day.dayName}
              </div>
              <div className="flex items-center mx-4">
                <i className={`${day.icon} text-lg w-6`}></i>
                <span className="ml-2 text-sm text-white/70">{day.description}</span>
              </div>
            </div>
            ...
            <div className="flex items-center space-x-4">
              ...
              <div className="flex items-center space-x-2 min-w-[80px] justify-end">
                <span className="text-sm font-medium text-white">{day.tempHigh}{temperatureSymbol}</span>
                <span className="text-sm text-white/60">{day.tempLow}{temperatureSymbol}</span>
              </div>
            </div>
          </motion.div>
        ))}
